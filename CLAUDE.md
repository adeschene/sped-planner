...
- **Bootstrap JS only started executing on 2026-08-18.** The `integrity` hash on the `bootstrap.bundle.min.js` tag in `layouts/application.html.erb` was corrupted, so browsers silently refused to run the script — for the entire life of the deploy up to that date. Nothing depended on it (collapse bodies render with `show` already applied; every form was a plain HTML POST), so it went unnoticed until the add-activity modal became the first `data-bs-*` feature. Consequences worth remembering: the day/week timeslot **collapse toggles never actually collapsed before that date**, and any pre-2026-08-18 report of "the JS doesn't work" is explained by this, not by the feature under suspicion. If a CDN asset's behavior ever goes missing again, verify the hash against the served bytes (`curl -sSfL <url> | openssl dgst -sha384 -binary | openssl base64 -A`) before debugging the feature — a sibling tag that verifies correctly isolates the fault to one attribute.
...
- **Don't re-add `parallelize` to `test/test_helper.rb`.** It was removed on 2026-08-18. `parallelize(with: :threads)` shares one database across workers, so concurrent `setup_fixtures` calls race to insert identical fixture ids and fail intermittently with `PG::UniqueViolation` on `activities_pkey`. The suite is ~90 tests in ~6s — there is nothing to gain and a flaky CI signal to lose.
- **System tests need an explicit sync point after login.** `click_button "Log In"` returns before the redirect lands; a following `visit` will outrun it and leave you back on `/login`, which then surfaces as a confusing missing-element error somewhere later. Assert on `"Welcome back!"` (the notice `SessionsController#create` sets) before navigating on.
...
| k | **Add-activity modal fields lack `required`** — title/block/date accept empty submits, caught only by the model, which bounces the user via `redirect_back` with a generic alert | Add `required: true` in `_add_activity_modal.html.erb`; flagged during the 2026-08-18 review and deferred |
| l | **Bootstrap JS behavior across the app is only lightly verified** — it first executed 2026-08-18 (see Stack), so every `data-bs-*` interaction outside the add-activity modal went live without ever having run before | Sweep header/nav dropdowns and the day/week collapse toggles when convenient |
